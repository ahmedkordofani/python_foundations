...
class Person:
    def __init__(self, data):
        ...
        self.addresses = {item['name']: f"{item['building']} {item['street']}" for item in data.get('addresses', [])}

    def get_work_address(self):
        return self.addresses.get('work', '')

    def get_home_address(self):
        return self.addresses.get('home', '')
    ...
    def get_pets(self):
        ...
        if num_pets == 0:
            ...
            pet_summary = f"{self.name} has {num_pets} pet{'s' if num_pets > 1 else ''}: "
            pet_details = [f"a {pet['animal']} called {pet['name']}" for pet in self.pets]
            return pet_summary + ', '.join(pet_details)
