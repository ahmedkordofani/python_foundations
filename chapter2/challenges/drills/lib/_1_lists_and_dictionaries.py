...
def we_joined_the_beatles(names_to_add):
    the_beatles = ['john', 'paul', 'george', 'ringo']
    the_beatles.extend(names_to_add)
    return the_beatles
...
#   Returns: ["a", "b", "c", "d"]
def add_to_list(input_list, element_to_add):
...
def new_band_member(new_member_info):
    existing_band = {"vocalist": "miss piggy", "lead_guitar": "scooter"}
    existing_band.update(new_member_info)
...
def touch_in(station, timestamp):
    return {'entrypoint': station, 'time': timestamp}
